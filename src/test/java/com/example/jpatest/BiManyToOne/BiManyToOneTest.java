...
/**
 * DataJpaTest 는 실제로 DB 까지 안감
 * 모두 ORM 과 데이터베이스의 중간다리 역할을 하는 영속성 컨텍스트 안에서 노는거임
 * teamRepository.save(team) 하면 영속성 컨텍스트 안에 저장하고
 * teamRepository.findById() 하면 영속성 컨텍스트 안에서 찾는거임
 */
...
public class BiManyToOneTest {
    ...
    @Test
    @DisplayName("Member 의 Team 연관관계 교체 성공")
    ...
    @Test
    @DisplayName("연관관계 편의 메서드 사용O : Member 추가 성공")
    ...
    void cascadeSaveInTeamSuccess(){
         ...
         * cascade all 이므로 member 들도 영속성 컨텍스트에 저장됨
         */
        teamRepository.save(team);

        // then
        // member 들을 영속성 컨텍스트에서 조회하면 memberRepository.save() 를 안했어도 cascade 덕에 조회가 됨!
    ...
    @Test
    @DisplayName("Cascade 의 remove 속성 사용 - Team 삭제 시 연관된 Member 자동 삭제")
